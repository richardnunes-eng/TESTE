...
let dashboardsInitialized = false;
...
const LOADER_QUOTES = [
    { text: "Tu te tornas eternamente responsável por aquilo que cativas.", author: "Antoine de Saint-Exupéry", book: "O Pequeno Príncipe" },
    { text: "O essencial é invisível aos olhos.", author: "Antoine de Saint-Exupéry", book: "O Pequeno Príncipe" },
    { text: "Só uma coisa torna um sonho impossível: o medo de fracassar.", author: "Paulo Coelho", book: "O Alquimista" },
    { text: "Os olhos mostram a força da alma.", author: "Paulo Coelho", book: "O Alquimista" },
    { text: "Tudo o que temos de decidir é o que fazer com o tempo que nos é dado.", author: "J.R.R. Tolkien", book: "O Senhor dos Anéis" },
    { text: "Estratégia sem tática é o caminho mais lento para a vitória. Tática sem estratégia é o ruído antes da derrota.", author: "Sun Tzu", book: "A Arte da Guerra" }
];

// ============================================================================
// UTILITÁRIOS DE DATA (FILTRO INICIAL)
// ============================================================================
...
function getOldestOpenDateIso(drivers) {
    let minIso = null;
    if (!Array.isArray(drivers)) return null;
    const eligibleStatus = new Set(['EM_ROTA', 'CRITICO']);
    drivers.forEach(d => {
        const statusNorm = String(d.status || '').toUpperCase();
        if (!eligibleStatus.has(statusNorm)) return;
        const iso = d.dataSaidaIso;
        if (!iso) return;
...
function applyDefaultDateRangeForOpen(drivers, force = false) {
    const dateStartEl = document.getElementById('dateStart');
    const dateEndEl = document.getElementById('dateEnd');
    if (!dateStartEl || !dateEndEl) return;

    if (!force && (dateStartEl.value || dateEndEl.value)) return;

    const todayIso = getLocalIsoDate(new Date());
    const oldestOpenIso = getOldestOpenDateIso(drivers);

    if (oldestOpenIso) {
        dateStartEl.value = oldestOpenIso;
        dateEndEl.value = todayIso;
    } else {
        // Sem itens em aberto com data: mantém início vazio e limita até hoje
        if (!dateStartEl.value) dateStartEl.value = '';
        dateEndEl.value = todayIso;
    }
}
...
window.onload = function() {
    ...
    setInterval(() => {
        console.log("📡 Auto-Sync (5 min): Verificando relatórios...");
        loadData('check'); 
    }, 300000);
...
// ============================================================================
// SISTEMA DE SUPRESSÃO TEMPORÁRIA (5 MINUTOS)
// ============================================================================
...
// ============================================================================
// SUBSTITUA A FUNÇÃO loadData NO JS-Logica.html POR ESTA:
// ============================================================================

function loadData(modo = 'check') {
    
    // Só bloqueia a tela se for manual (Botão)
    if (modo === 'force') {
        showGlobalLoader('Sincronizando dados...');
    }
    
    // Ícone gira em todos os modos
    const icon = document.getElementById('refreshIcon');
    if(icon) icon.classList.add('rotating');

    // Timeout de segurança - 30s
    const timeoutId = setTimeout(() => {
        console.warn("⚠️ Timeout: Requisição demorou demais");
    ...
    google.script.run
        .withSuccessHandler(data => {
            ...
            // ✅ CORREÇÃO: Verifica se data existe antes de acessar propriedades
            if (!data) {
            ...
            incomingDrivers.forEach(d => {
                ...
                const finalizados = ['concluido', 'concluído', 'fechado', 'done', 'finalizada', 'finalizado'];
            ...
            // Define filtro padrão: do item em aberto mais antigo até hoje
            applyDefaultDateRangeForOpen(state.allDrivers);
            ...
            if(label) label.innerText = `Atualizado: ${dataFmt} às ${horaFmt}`;

            initialDataLoaded = true;
            renderDashboard({ drivers: state.allDrivers, silent: (modo !== 'force') }); 
            
            // Para o ícone e esconde loader
            if(icon) icon.classList.remove('rotating');
...
function initLoaderFailsafe() {
    ...
    if (quoteEl) quoteEl.innerText = 'Atualizando informações...';
    startLoaderQuotes();
}
...
function showGlobalLoader(msg, mode = 'hard') {
    ...
    // Segurança: Fecha sozinho se travar por 15s
...
function setLoaderQuote() {
    ...
    authorEl.innerText = authorBits.length ? `— ${authorBits.join(", ")}` : "";
}
...
// ============================================================================
function applyPipeline(showLoader = true, loaderMode = 'hard') {
    // Mostra o loading APENAS se showLoader for true
    if (showLoader) {
        const loader = document.getElementById('globalLoader');
        if(!loader || loader.style.display === 'none') {
            showGlobalLoader('Atualizando visualização...', loaderMode);
        }
    }
      ...
      // --- FILTRO DE BUSCA ---
            if (state.searchTerms.length > 0) {
                ...
                result = result.filter(d => {
                    ...
                    // OR lógico: algum termo precisa bater
...
function exportCsv() {
    ...
    showGlobalLoader('Gerando planilha...');
    ...
    google.script.run
        .withSuccessHandler(res => {
            ...
            if (res && (res.downloadUrl || res.url)) {
                // Evita bloqueio de pop-up; inicia download na mesma aba.
                window.location.href = res.downloadUrl || res.url;
                return;
            }
        ...
        .withFailureHandler(err => {
            ...
            exportInProgress = false;
            if (btn) btn.removeAttribute('disabled');
            alert('Erro ao exportar planilha: ' + err);
        })
        .exportDashboardCsv();
}
...
function renderGrid(list) {
    ...
    if (list.length === 0) {
        grid.innerHTML = `<div style="grid-column: 1 / -1; text-align: center; padding: 80px 0; color: var(--text-muted);"><span class="material-icons-round" style="font-size: 4rem; opacity: 0.3; margin-bottom: 10px;">search_off</span><h3>Nenhum veículo encontrado</h3><p>Tente ajustar os filtros ou a busca.</p></div>`;
        return;
    }
    ...
    list.forEach(d => {
        ...
        // Lógica de alerta do ClickUp
        let clickupAlertHtml = '';
        if ((d.statusClass === 'success' || d.statusClass === 'warning')) {
            const statusFechados = ['concluido', 'concluído', 'fechado', 'done', 'complete', 'finalizado', 'cancelado', 'finalizada'];
        ...
        <div class="card-driver" onclick="openModal('${safeId}')">
            <div class="card-header">
                <div style="display:flex; gap:12px; align-items:center;">
                    ...
                    <div class="info">
                        <h3>${d.motorista}</h3>
                        <div class="info-sub">
                            <span><span class="material-icons-round icon-small">local_shipping</span> ${d.veiculo} • ${d.placa}</span>
            ...
            <div style="margin-top:auto;">
                ...
                <div class="progress-legend">
                    ...
                    <span>Devolução: ${devPct}%</span>
                </div>
                
                <div class="stats-row" style="margin-top: 8px;">
                    <span>${d.entregas} Entregas • ${d.peso} kg</span>
...
function renderTable(list) {
    ...
    list.forEach(d => {
        ...
        let clickupIcon = '';
        if ((d.statusClass === 'success' || d.statusClass === 'warning')) {
            const statusFechados = ['concluido', 'concluído', 'fechado', 'done', 'finalizada', 'finalizado'];
            const stClickup = (d.statusClickup || '').toLowerCase();
...
function getDriverDelay(driver) {
    // Se não houver detalhes de paradas, não há como calcular
    if (!driver.detalhes || driver.detalhes.length === 0) return 0;
    
    // Procura a primeira parada pendente que tenha um horário de chegada registrado
    let currentStop = driver.detalhes.find(s => !s.isDone && !s.isDev && s.hora && s.hora !== '--:--');
    
    if (!currentStop || !currentStop.hora) return 0;
    
    // Calcula a diferença entre agora e o horário que ele chegou no cliente
    let now = new Date();
...
function buildLiveStatus(d, mode) {
    ...
    return `<span class="tbl-live-info ${dangerClass}">📍 ${currentStop.cliente.substring(0, 15)}... (${diffMins} min)</span>`;
}
...
// ============================================================================
function initDashboardsControls() {
    if (dashboardsInitialized) return;
    const unidadeEl = document.getElementById('dashUnidade');
    const statusEl = document.getElementById('dashStatus');
    const windowEl = document.getElementById('dashWindow');
    if (!unidadeEl || !statusEl || !windowEl) return;
    unidadeEl.addEventListener('change', renderDashboardsSection);
    statusEl.addEventListener('change', renderDashboardsSection);
    windowEl.addEventListener('change', renderDashboardsSection);
...
function renderDashboardsSection() {
    ...
    if (!section) return;
    initDashboardsControls();

    const drivers = Array.isArray(state.allDrivers) ? state.allDrivers : [];
    populateDashUnidadeOptions(drivers);

    const filters = getDashFilters();
    const filtered = filterDashDrivers(drivers, filters);

    updateDashKpis(filtered);
    renderDashStatusDonut(filtered);
    renderDashTrend(filtered, filters.windowDays);
    renderDashTempoRota(filtered);
    renderDashCriticalList(filtered);
    renderDashTopDrivers(filtered);
    renderDashUnitBars(filtered);
}

function populateDashUnidadeOptions(drivers) {
    const unidadeEl = document.getElementById('dashUnidade');
    if (!unidadeEl) return;
    const prev = unidadeEl.value;
    const unidades = [...new Set(drivers.map(d => d.unidade).filter(Boolean))].sort();
    unidadeEl.innerHTML = '<option value="">Todas as Unidades</option>';
    unidades.forEach(u => {
        const opt = document.createElement('option');
        opt.value = u;
        opt.textContent = u;
        unidadeEl.appendChild(opt);
    });
    unidadeEl.value = unidades.includes(prev) ? prev : '';
}
...
function getDashFilters() {
    ...
    const windowDays = windowEl ? parseInt(windowEl.value || '7', 10) : 7;
    return { unidade, status, windowDays: Math.max(3, Math.min(windowDays, 30)) };
}

function filterDashDrivers(drivers, filters) {
    ...
    if (filters.status && filters.status !== 'ALL') {
        result = result.filter(d => {
            const statusNorm = String(d.status || '').toUpperCase();
            if (filters.status === 'EM_ROTA') return statusNorm === 'EM_ROTA';
            if (filters.status === 'CRITICO') return statusNorm === 'CRITICO' || getDriverDelay(d) > 30;
            if (filters.status === 'FINALIZADO') return statusNorm === 'FINALIZADO' || statusNorm === 'RESSALVA';
            return true;
        });
    }
    return result;
...
function updateDashKpis(data) {
    const totals = getDashStatusTotals(data);
    const totalEl = document.getElementById('dashKpiTotal');
    const emRotaEl = document.getElementById('dashKpiRota');
    const criticoEl = document.getElementById('dashKpiCrit');
    const finalEl = document.getElementById('dashKpiFinal');
    if (totalEl) totalEl.textContent = totals.total;
    if (emRotaEl) emRotaEl.textContent = totals.emRota;
    if (criticoEl) criticoEl.textContent = totals.critico;
    if (finalEl) finalEl.textContent = totals.finalizado;
}

function getDashStatusTotals(data) {
    const totals = { total: 0, emRota: 0, critico: 0, finalizado: 0, outros: 0 };
    data.forEach(d => {
        totals.total++;
        const statusNorm = String(d.status || '').toUpperCase();
        if (statusNorm === 'EM_ROTA') totals.emRota++;
        else if (statusNorm === 'CRITICO' || getDriverDelay(d) > 30) totals.critico++;
        else if (statusNorm === 'FINALIZADO' || statusNorm === 'RESSALVA') totals.finalizado++;
        else totals.outros++;
    });
    return totals;
}

function renderDashStatusDonut(data) {
    const donut = document.getElementById('dashStatusDonut');
    const legend = document.getElementById('dashStatusLegend');
    const totalEl = document.getElementById('dashStatusTotal');
    ...
    if (total === 0) {
        donut.style.background = 'conic-gradient(var(--input-border) 0 360deg)';
        legend.innerHTML = '<div class="dash-empty">Sem dados para o filtro atual.</div>';
        return;
    }

    const segments = [
        { key: 'EM_ROTA', label: 'Em rota', value: totals.emRota, color: 'var(--primary)' },
        { key: 'CRITICO', label: 'CrÃ­ticos', value: totals.critico, color: 'var(--danger)' },
        { key: 'FINALIZADO', label: 'Finalizados', value: totals.finalizado, color: 'var(--accent)' },
        { key: 'OUTROS', label: 'Outros', value: totals.outros, color: 'var(--text-muted)' }
    ];
    ...
    legend.innerHTML = segments.map(seg => {
        ...
        const keyValue = seg.key === 'OUTROS' ? 'ALL' : seg.key;
        return `
            <button class="dash-legend-item" onclick="setDashStatusFilter('${keyValue}')">
...
function setDashStatusFilter(value) {
    const statusEl = document.getElementById('dashStatus');
    if (!statusEl) return;
...
function renderDashTrend(data, windowDays) {
    ...
    for (let i = windowDays - 1; i >= 0; i--) {
        ...
        const bucket = { iso, label, count: 0 };
        buckets.push(bucket);
        bucketMap.set(iso, bucket);
    }

    data.forEach(d => {
        if (!d.dataSaidaIso) return;
        const bucket = bucketMap.get(d.dataSaidaIso);
    ...
    trendEl.innerHTML = buckets.map(b => {
        ...
        return `
            <div class="bar-col" title="${b.label} â€¢ ${b.count}">
...
function renderDashTempoRota(data) {
    const fillEl = document.getElementById('dashTempoMeterFill');
    const metaEl = document.getElementById('dashTempoMeta');
    if (!fillEl || !metaEl) return;
    const tempos = data.map(d => parseTempoRotaMinutes(d.tempoRota)).filter(v => v > 0);
    if (tempos.length === 0) {
        fillEl.style.width = '0%';
        metaEl.textContent = 'Sem dados de tempo em rota.';
        return;
    }
    ...
    metaEl.textContent = `MÃ©dia: ${formatMinutes(avg)} â€¢ Pico: ${formatMinutes(max)} â€¢ Meta: 8h`;
}

function renderDashCriticalList(data) {
    const listEl = document.getElementById('dashCriticalList');
    if (!listEl) return;
    ...
    if (criticos.length === 0) {
        listEl.innerHTML = '<div class="dash-empty">Nenhuma rota crÃ­tica no filtro atual.</div>';
        return;
    }
    criticos.sort((a, b) => getDriverDelay(b) - getDriverDelay(a));
    listEl.innerHTML = criticos.slice(0, 5).map(d => {
        const delay = getDriverDelay(d);
        const delayText = delay > 0 ? `${delay} min parado` : 'Sem permanÃªncia';
        return `
            <div class="dash-list-item">
                <div>
                    <div class="dash-list-title">${d.motorista || 'Sem motorista'}</div>
                    <div class="dash-list-sub">${d.plano || '--'} â€¢ ${d.unidade || 'N/A'}</div>
                </div>
                <span class="dash-pill danger">${delayText}</span>
            </div>
        `;
    }).join('');
}

function renderDashTopDrivers(data) {
    const listEl = document.getElementById('dashTopDrivers');
    if (!listEl) return;
    const map = new Map();
    data.forEach(d => {
        const key = d.motorista || 'Sem motorista';
        const entregas = Number(d.entregas) || 0;
        const current = map.get(key) || { entregas: 0, rotas: 0 };
        current.entregas += entregas;
        current.rotas += 1;
        map.set(key, current);
    });
    const items = [...map.entries()].sort((a, b) => b[1].entregas - a[1].entregas).slice(0, 5);
    if (items.length === 0) {
        listEl.innerHTML = '<div class="dash-empty">Sem dados de motoristas.</div>';
        return;
    }
    const max = Math.max(...items.map(i => i[1].entregas), 1);
    listEl.innerHTML = items.map(([name, info]) => {
        const pct = Math.round((info.entregas / max) * 100);
        return `
            <div class="dash-list-item">
                <div>
                    <div class="dash-list-title">${name}</div>
                    <div class="dash-list-sub">${info.entregas} entregas â€¢ ${info.rotas} rotas</div>
                </div>
...
function renderDashUnitBars(data) {
    const chartEl = document.getElementById('dashUnitBars');
    if (!chartEl) return;
    const map = new Map();
    data.forEach(d => {
        const key = d.unidade || 'N/A';
        const valor = Number(d.valorTotal) || 0;
        const current = map.get(key) || 0;
        map.set(key, current + valor);
    });
    const items = [...map.entries()].sort((a, b) => b[1] - a[1]).slice(0, 8);
    if (items.length === 0) {
        chartEl.innerHTML = '<div class="dash-empty">Sem dados de unidades.</div>';
        return;
    }
    const max = Math.max(...items.map(i => i[1]), 1);
    chartEl.innerHTML = items.map(([unit, value]) => {
        const pct = Math.round((value / max) * 100);
        const valFmt = value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
        return `
            <div class="hbar-row">
                <div class="hbar-label">${unit}</div>
                <div class="hbar-track"><span style="width:${pct}%"></span></div>
                <div class="hbar-value">${valFmt}</div>
            </div>
        `;
    }).join('');
}
...
function switchSection(sectionId, menuItem) {
    const sections = ['dashboard', 'occurrence', 'dashboards'];
    sections.forEach(id => {
    ...
    if (sectionId === 'dashboards') renderDashboardsSection();
}
...
function setClickupStatus(novoStatus) {
    ...
    if (!driver) { alert("Erro: Motorista não encontrado."); return; }
    if (!driver.clickupId) { alert("Erro: Esta rota não tem ID do ClickUp vinculado."); return; }
...
function actionClickup() {
    ...
    if(driver && driver.clickupId) {
        ...
        alert("⚠️ ID do ClickUp não encontrado para esta rota.");
    }
}
...
function actionAdiantamento() {
    const valor = prompt("💰 Qual o valor do adiantamento (R$)?");
    if(valor) {
        alert(`Solicitação de R$ ${valor} enviada para o financeiro!`);
    }
}

function actionDebito() {
    const valor = prompt("🧾 Qual o valor do débito (R$)?");
    if (valor) {
        alert(`Débito de R$ ${valor} lançado com sucesso!`);
    }
}
...
window.renderDetailedModal = function(id) {
    ...
    if (!driver.detalhes || driver.detalhes.length === 0) {
        ...
        sortedStops.forEach((s, index) => {
            ...
            let horarioTexto = "--:--";
            if (s.hora && s.hora !== '--:--') {
                if (s.saida && s.saida !== '--:--') horarioTexto = `${s.hora} <span style="font-size:10px; color:var(--text-muted)">➔</span> ${s.saida}`;
                else horarioTexto = `${s.hora} <span style="font-size:10px; color:var(--text-muted)">➔</span> ...`;
            }
